...
<div class="header-text text-center header-image-electrical" style="padding-top:220px">
    <h2 style="font-size:35px;min-height:40px;color:white" id="text-shadow4"></h2>
  </div>
            ...
            <div clas="row">
                <div class="col-sm-7" style="padding:0;margin:0">
                    <div class="startern-header-div" style="padding-top:30px">
                        ...
                        <h1 class="startern-header"><span >Electrical Service</span> </h1>
                ...
                <form method="post" action="{{ url('/all/service/add') }}" enctype="multipart/form-data">
                  ...
                  <select class="form-control" id="sel1" name="selected_service_area">
                    ...
                    @foreach ($electrician_area as $electrician)
                      <option value="{{ $electrician->id }}">{{ $electrician->sub_ser_area }}</option>
                    @endforeach
                  </select>
                </div>
                  <div class="form-group">
                      <select class="form-control" id="sel1" name="selected_sub_service">
                        ...
                        @foreach ($electrician_info as $electrician)
                          <option value="{{ $electrician->id }}">{{ $electrician->sub_service }}</option>
                        @endforeach
                      </select>
                    </div>
                    <div class="form-group">
