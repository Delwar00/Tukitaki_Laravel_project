...
<div class="header-text text-center header-image-baby" style="padding-top:220px">
    <h2 style="font-size:35px;min-height:40px;color:white" id="text-shadow1">
	</h2>
  </div>
            ...
            <div clas="row">
                <div class="col-sm-7" style="padding:0;margin:0">
                    <div class="startern-header-div" style="padding-top:30px">
                        <h1 class="startern-header"><span >Baby Sitting Service</span> </h1>
                    </div>
              ...
              <form method="post" action="{{ url('/all/service/add') }}" enctype="multipart/form-data">
                @csrf
                  ...
                  <select class="form-control" id="sel1" name="selected_service_area">
                    ...
                    @foreach ($babysitting_area as $baby)
                      <option value="{{ $baby->id }}">{{ $baby->sub_ser_area }}</option>
                    @endforeach
                  </select>
                </div>
                  <div class="form-group">
                      <select class="form-control" id="sel1" name="selected_sub_service">
                        ...
                        @foreach ($babysitting_info as $baby)
                          <option value="{{ $baby->id }}">{{ $baby->sub_service }}</option>
                        @endforeach
                      </select>
                    </div>
                    <div class="form-group">
