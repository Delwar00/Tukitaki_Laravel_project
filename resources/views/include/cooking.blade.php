...
<div class="header-text text-center header-image-cooking" style="padding-top:220px">
   <h2 style="font-size:35px;min-height:40px;color:white" id="text-shadow"></h2>
 </div>
 </header>
              ...
              <form method="post" action="{{ url('/all/service/add') }}" enctype="multipart/form-data">
                @csrf
                 ...
                 <select class="form-control" id="sel1" name="selected_service_area">
                   ...
                   @foreach ($cooking_area as $cooking)
                     <option value="{{ $cooking->id }}">{{ $cooking->sub_ser_area }}</option>
                   @endforeach
                 </select>
               </div>
                 ...
                 <div class="form-group">
                     <select class="form-control" id="sel1" name="selected_sub_service">
                       ...
                       @foreach ($cooking_info as $cooking)
                         <option  value="{{ $cooking->id }}">{{ $cooking->sub_service }}</option>
                       @endforeach
                     </select>
                   </div>
                   <div class="form-group">
